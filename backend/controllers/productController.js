  ...
  try {
    ...
    if (category) {
      try {
        ...
        if (catData) {
          query = query.ilike('category', catData.name);
        } else {
    ...
    if (error) {
      console.warn('DB error fetching products:', error.message);
    ...
    const formattedProducts = (products || []).map(p => ({
      ...
      specificationPdfUrl: p.specification_pdf_url,
...
const getProductById = async (req, res) => {
  try {
    ...
    // Fetch description images
    const { data: descImages } = await db.database.from('product_description_images').select('*').eq('product_id', product.id).order('sort_order', { ascending: true });
    const descriptionImagesArr = (descImages || []).map(di => di.image_url).filter(Boolean);
    ...
    res.json({
      ...
      specificationPdfUrl: product.specification_pdf_url,
      ...
      images: imagesArr,
      descriptionImages: descriptionImagesArr,
      unit: product.unit,
...
const createProduct = async (req, res) => {
  const {
    ...
    isFlashSale, isDigital, digitalFileUrl, specificationPdfUrl, metaTitle, metaDescription, metaKeywords, metaImage, tags, youtubeUrl, flashSaleStart, flashSaleEnd,
  ...
  try {
    ...
    while (result.error && result.error.message && (
      result.error.message.includes('column') && 
      (result.error.message.includes('does not exist') || result.error.message.includes('Could not find the'))
    )) {
      let match = result.error.message.match(/column "([^"]+)" of relation/);
      if (!match) match = result.error.message.match(/'([^']+)' column/);
    ...
    const descriptionImages = req.body.descriptionImages;
    if (descriptionImages && Array.isArray(descriptionImages) && descriptionImages.length > 0) {
      const descImgRows = descriptionImages.filter(Boolean).map((url, i) => ({
        product_id: product.id,
        image_url: url,
        sort_order: i,
      }));
      const { error: descImgErr } = await db.database.from('product_description_images').insert(descImgRows);
      if (descImgErr) console.error('Failed to save description images:', descImgErr);
    }
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (specificationPdfUrl !== undefined) updateData.specification_pdf_url = specificationPdfUrl;
    ...
    // Save description images if provided
    const descriptionImages = req.body.descriptionImages;
    if (descriptionImages !== undefined && Array.isArray(descriptionImages)) {
      await db.database.from('product_description_images').delete().eq('product_id', req.params.id);
