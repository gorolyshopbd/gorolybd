...
export const getPageBySlug = async (req, res) => {
  try {
    const { data: page, error } = await supabase.from('pages').select('*').eq('slug', req.params.slug).eq('is_published', true).single();
...
export const updatePage = async (req, res) => {
  try {
    ...
    const { data: page, error } = await supabase.from('pages').update(updateData).eq('id', req.params.id).select().single();
...
export const getPublicPages = async (req, res) => {
  try {
    const { data: pages, error } = await supabase.from('pages').select('*').eq('is_published', true).order('created_at', { ascending: false });
    if (error) throw error;
