...
# Update DB to Simulated
sftp = client.open_sftp()
with sftp.open('/tmp/fix_otp.sql', 'w') as f:
    f.write("UPDATE settings SET otp_gateway = 'Simulated' WHERE id = (SELECT id FROM settings LIMIT 1);\n")
sftp.close()
_, stdout, _ = client.exec_command("docker exec -i shopio-db psql -U postgres -d shopio < /tmp/fix_otp.sql")
print("DB:", stdout.read().decode()[:100])
...
print("settings verify:", stdout.read().decode().strip())

client.close()
print("\nSimulated mode active - OTP will show in response")
