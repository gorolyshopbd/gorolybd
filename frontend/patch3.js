...
const regex = /\\s*<\\/div>\\s*value=\\{settings\\.sasSmsGatewayUrl \\|\\| ''\\}/;
const replacement = `
              </div>
...
if (new RegExp(regex.source).test(c)) {
  fs.writeFileSync('src/components/AdminDashboard.js', c.replace(new RegExp(regex.source), replacement));
